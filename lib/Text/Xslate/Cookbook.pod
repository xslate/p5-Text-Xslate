...
with templates by youself. Basically there are two basic solutions in Xslate:
use of FillInForm or HTML form builders.

In both solutions, one should not use C<mark_raw> in templates because
of itsB<security risks>. Instead, application code should be responsible for
it.
