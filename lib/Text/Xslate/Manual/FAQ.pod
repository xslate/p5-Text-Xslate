...
=head3 Optimizations Employed By Text::Xslate
...
However, if you want to process binary strings, you can do by passing C<:raw>
for C<input_encoding>.

=head3 Why doesn't $object.attr access the hash element?

Template-Toolkit allows objects (i.e. blessed references) to access its element, i.e. C<< [% object.attr %] >> means C<< $object->{attr} >> if the object has no C<attr> method. This behavior breaks encapsulation and hides typos, so Xslate doesn't take this strategy.
...
=head3 How can I use Xslate into my favorite WAF?
