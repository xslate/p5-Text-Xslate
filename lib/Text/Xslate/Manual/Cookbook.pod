...
Some CPAN module might hep you. See L<String::Filter> for example.
...
You can override C<slurp_template()> in Text::Xslate to process contents
before parsing them:
...
The first argument I<$self> is an Text::Xslate instance; the second is
the argument of C<input_layer> option (default to C<:utf8>); the third
is the full path of a template file to load.

C<slurp_template> may return both B<text strings> and C<byte strings>.

Note that this hook is provided for pre-processing template contents, so you should calll C<SUPER::slurp_template> to load templates into the memory.
